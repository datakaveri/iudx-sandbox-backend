...
func stopNotebook(app *application.Application) httprouter.Handle {
	return func(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
		...
		_, err = jupyterClient.StopServer(app, tokenUser.UserName, spawnerName)
		...
		if err != nil {
			...
			w.WriteHeader(http.StatusInternalServerError)
			logger.Error.Printf("Error in stopping notebook %v\n", err)
			return
		}
...
func Do(app *application.Application) httprouter.Handle {
	return middleware.Chain(stopNotebook(app), middleware.LogRequest, middleware.AuthorizeRequest)
}
