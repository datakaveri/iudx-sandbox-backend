...
func getRepo(app *application.Application) httprouter.Handle {
	return func(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
		repoName := strings.TrimPrefix(r.URL.Path, "/api/gallery/")

		repo := &models.Repo{}
		res, err := repo.Get(app, repoName)
		...
		if err != nil {
			...
			logger.Error.Printf("Error in fetching Gallery Repo %v\n", err)
			return
		}

		w.Header().Set("Content-Type", "application/json")
		newResponse := apiresponse.New("success", "Details of Gallery Repo")
...
func Do(app *application.Application) httprouter.Handle {
	return middleware.Chain(getRepo(app), middleware.LogRequest)
}
